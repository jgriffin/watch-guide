...
- [x] **Guides built:** one folder per match under `matches/` — that directory **is** the count and the list (don't enumerate it here; it drifts). To see what's built: `ls matches/`, or run `./build-site.sh` (its banner prints `Built site/ with N guides:` + every slug).

...
- `bracket.json` shape: `rounds[]` (r32/r16/qf/sf/final) of ordered `ties` + `third`. Per tie: `num, date, t1/t2, f1/f2 (flag emoji), s1/s2, winner, note (pens/AET), feeders[2], slug`. **Topology is fixed** (canonical 2026 feeder map baked into `fetch-bracket.sh`, not read from openfootball's laggy placeholder-resolution). Downstream TBD slots are **filled from feeder winners** in the fetch (one level/fetch, cascades round by round). Flags are baked in `fetch-bracket.sh`; **slug is derived** as `slugify(t1)-slugify(t2)` with a 2-entry override map (M73→`canada-south-africa`, M80→`england-congo-dr`, where the folder order/spelling differs) — it is **no longer read from `fixtures.json`** (that dependency was removed so `fetch-bracket.sh` can now *generate* `fixtures.json`; see next). `fillTie` recomputes the slug after propagating feeder winners, so filled QF/SF slots link too.

### Schedule & fixtures — `schedule.json` (calendar SoT) → generated `fixtures.json`
- **`schedule.json`** is the hand-maintained **single source of truth for the knockout calendar**: one entry per FIFA match number (M73–M104) with `{round, date, ko_pacific, ko_display, venue}` — **team-independent** (venues/times are fixed the moment the tournament schedule is published; only *which teams* fill each slot changes as the bracket resolves). R32/R16 were bootstrapped from the original hand-authored fixtures; QF-onward researched (ESPN + FIFA/SI). **To change a venue/time, edit `schedule.json` — nothing else.**
- **`fixtures.json` is now GENERATED** by `fetch-bracket.sh` (a second `jq` pass after `bracket.json`): every knockout tie whose two teams are known → a fixture row joining `schedule.json` (venue/time) with the resolved bracket (teams + derived slug), sorted by date. This **kills the per-round manual toil** — as the bracket advances, `fixtures.json` auto-extends (and the existing bracket-refresh cron regenerates it 4×/day). **Do not hand-edit `fixtures.json` or `bracket.json`** — both are generated + committed. Group-stage guides live outside the bracket and remain "orphan" guides in `build-site.sh`.

### Filling missing guides — `/watch-guide-sync` skill
- **`/watch-guide-sync`** (`skills/watch-guide-sync/SKILL.md`, symlinked into `~/.claude/skills/`) is the one-command catch-up: runs `fetch-bracket.sh`, finds **upcoming fixtures with no guide** (default scope: today + next match day; `all` / a slug / a date widen it), generates each via a subagent following `watch-guide-prompt.md` (spine pre-filled from `fixtures.json`/`bracket.json`, incl. the derived "What's Next" opponent; played games → retrospective), rebuilds, and pushes to `main`. Publish gate = **straight to main** (auto-deploy). Guide generation is the LLM-hard part, so it's local/user-triggered — not in the cron. Can be wrapped in a local `/loop`/cron later for hands-off runs.
...
### Site / redeploy
- Build: `./build-site.sh` regenerates `site/` from `matches/` + `fixtures.json` + `bracket.json`. Copies each guide → `site/<slug>/index.html` **as-is (no snippet)**; renders the schedule landing + the bracket; appends the hide-scores snippet to the landing only. `fixtures.json` is **generated** (from `schedule.json` ⋈ bracket — see above), so a resolved-but-unbuilt game shows a **placeholder** automatically; no manual refresh. Each card's round label comes from the fixture's `round` field. Landing order: **Today** (open) → **Next up** (open) → **More upcoming** (`<details>`) → **the bracket** (connected R32→Final tree; horizontally scrollable on mobile) → **Earlier games** (`<details>`, + orphan group guides). `.sheet` widened to 860px to fit the 5-column tree. `site/` is gitignored (derived); the committed source is `build-site.sh` + `fetch-bracket.sh` + `schedule.json` + generated `fixtures.json` + `bracket.json` + `matches/`.

...
jq -n \
  --slurpfile wc "$RAW" \
  --slurpfile fx "$ROOT/fixtures.json" \
  --arg gen "$GEN" '
  # --- normalize a team name to a canonical token for slug matching -----------
  def norm(s):
      (s|ascii_downcase|gsub("&";"")|gsub("[^a-z0-9]";"")) as $t
      | if ($t=="drcongo" or $t=="congodr") then "congo" else $t end;
  ...
  # fixtures.json -> { "<sorted normalized pair>": "<slug>" }
  | (reduce $fix[] as $f ({};
       . + { ([norm($f.home), norm($f.away)]|sort|join("|")): $f.slug }
    )) as $slugByPair
  ...
  | def tie($num; $feeders):
      ...
      | {
          ...
          slug: (if ($t1!=null and $t2!=null)
                 then ($slugByPair[[norm($t1), norm($t2)]|sort|join("|")] // null)
                 else null end)
        };
